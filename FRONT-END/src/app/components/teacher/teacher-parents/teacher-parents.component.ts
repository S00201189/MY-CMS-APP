...
const endpoint = `${environment.SERVER_URL}api/parents`;
...
export class TeacherParentsComponent implements OnInit {
  ...
  createParent() {
    const endpoints = `${environment.SERVER_URL}api/parents/create`;
  ...
  getParents() {
    const url = `${environment.SERVER_URL}api/parents`;
    this.http.get(url, httpOption).subscribe(
      (response: any) => {
        console.log(response);
  ...
  updateParent() {
    const endpoints = `${environment.SERVER_URL}api/parents/${this.updatedId}`;
  ...
  remove(id: any) {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        const endpoints = `${environment.SERVER_URL}api/parents/${id}`;
